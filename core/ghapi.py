    ...
    for attempt in range(retries + 1):
        try:
            ...
            if resp.status_code in (403, 429, 500, 502, 503, 504) and attempt < retries:
                ...
                logger.debug(f"[ghapi] {method} {url} -> {resp.status_code}, {delay}s 重试({attempt+1}/{retries})")
                time.sleep(delay)
                continue
            return resp.status_code, last_body
            ...
            if attempt < retries:
                delay = HTTP_RETRY_BACKOFF * (attempt + 1)
                logger.debug(f"[ghapi] {method} {url} 超时, {delay}s 重试({attempt+1}/{retries})")
                time.sleep(delay)
                continue
            logger.warning(f"[ghapi] {method} {url} 超时({retries+1}次)")
            return 0, "timeout"
        except requests.exceptions.ConnectionError as e:
            if attempt < retries:
                delay = HTTP_RETRY_BACKOFF * (attempt + 1)
                logger.debug(f"[ghapi] {method} {url} 连接错误: {e}, {delay}s 重试({attempt+1}/{retries})")
                time.sleep(delay)
                continue
            logger.warning(f"[ghapi] {method} {url} 连接失败({retries+1}次): {e}")
            return 0, str(e)
        except Exception as e:
            if attempt < retries:
                time.sleep(1)
                continue
            logger.error(f"[ghapi] {method} {url} 异常({retries+1}次): {e}")
            return 0, str(e)
...
def check_rate_limit(token=None, force=False):
    ...
    if status != 200:
        logger.warning(f"[ghapi] rate_limit 查询失败: {status}")
        return 0, 0, 0
...
def estimate_account_quota(account):
    ...
    try:
        ...
        try:
            ...
            logger.debug(f"[ghapi] 查询运行数失败: {e}")
        ...
        logger.warning(f"[ghapi] 估算配额失败: {e}")
